...
<div class="ml-64 p-8 w-full bg-gray-50 min-h-screen">
    ...
    <div class="flex justify-between items-center bg-white p-6 shadow-md rounded-lg mb-6">
...
@if (auth()->user() && auth()->user()->hasPermission('Create'))
    ...
    <!-- Products Table -->
    <div id="products-table-section" class="mt-6 bg-white p-6 shadow-lg rounded-lg">
        <h2 class="text-2xl font-bold text-gray-800 mb-4">Products</h2>
        <div class="overflow-auto max-h-[400px]">
            <table class="w-full bg-white shadow-md rounded-lg overflow-hidden">
                <thead class="bg-gray-800">
                    <tr>
                        <th class="p-4 text-left font-semibold text-gray-100">Image</th>
                        <th class="p-4 text-center font-semibold text-gray-100">SKU</th>
                        <th class="p-4 text-center font-semibold text-gray-100">Name</th>
                        <th class="p-4 text-center font-semibold text-gray-100">Quantity Left</th>
                        <th class="p-4 text-center font-semibold text-gray-100">Price</th>
                        <th class="p-4 text-center font-semibold text-gray-100">Cost Price</th>
                        <th class="p-4 text-center font-semibold text-gray-100">Category</th>
                        <th class="p-4 text-center font-semibold text-gray-100">Supplier Code</th>
                    </tr>
                </thead>
                <tbody id="product-table">
                    @foreach($products as $product)
                        <tr class="hover:bg-blue-50 transition duration-200">
                            ...
                            <td class="p-4 text-center text-gray-800">RM{{ number_format($product->price, 2) }}</td>
                            <td class="p-4 text-center text-gray-800">RM{{ number_format($product->cost_price, 2) }}</td>
...
<script>
    // Smooth scroll to products table and filter by category
    document.querySelectorAll('.category-btn').forEach(button => {
        button.addEventListener('click', () => {
            ...
            // Optional: Smooth scroll to products table after navigation
            // setTimeout(() => {
            //     document.getElementById('products-table-section').scrollIntoView({ behavior: 'smooth' });
            // }, 500);
        });
    });
</script>