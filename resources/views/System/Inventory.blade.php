        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-5">
            @foreach ($categories as $index => $category)
                <a href="{{ route('products.view', ['category' => $category]) }}" 
                   class="p-6 shadow-md rounded-lg text-center cursor-pointer transition-all duration-300
...
<div class="mt-4 flex justify-end">
    <a href="{{ route('products.create') }}" 
       class="bg-blue-500 text-white p-3 rounded-lg shadow-md hover:bg-blue-600 transition duration-200">
        ➕ Add Product
    </a>
</div>

    <!-- Products Table -->
    <div class="mt-6 overflow-auto max-h-[400px]">
        <table class="w-full bg-white shadow-md rounded-lg overflow-hidden">
            <thead class="bg-gray-200">
                <tr>
        <th class="p-3 text-left font-semibold text-gray-700">Image</th>
                    <th class="p-3 text-center font-semibold text-gray-700">SKU</th>
            ...
            <tbody id="product-table">
                @foreach($products as $product)
                    <tr class="product-row hover:bg-gray-100 transition duration-200" data-category="{{ $product->category }}">
                        <!-- Product Image-->
                    <td class="p-3">
                        @if($product->image)
